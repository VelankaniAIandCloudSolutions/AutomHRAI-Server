...
class Main():
    ...
    def _load_encodings_from_db(self) -> None:
        known_encodings, self.name, self.employee_ids, self.blacklisted, self.last_id, self.email_ids = self.db.get_all_embeddings()
    ...
    def update_cluster(self):
        if self.last_id > 0:
            new_encodings, new_users, new_employee_ids, blacklisted, self.last_id, email_id = self.db.get_all_embeddings(from_id=self.last_id)
            if new_users != []:
                ...
                self.email_ids = self.email_ids + email_id
    ...
    def match_embedding(self, embedding: list):
        if self.last_id > 0:
            ...
            if prediction != -1:
                ...
                email_id = self.email_ids[prediction]
            else:
                name = 'Unknown'
                employee_id = None
                email_id = None
        else:
            return 'Unknown', None, -1, 999, None
        return name, employee_id, prediction, distances[0][0], email_id
    ...
    def add_tracker(self, lane_id, track_id):
        ...
        self.current_trackers[lane_id][track_id]['face_email_id'] = []
        # self.current_trackers[lane_id][track_id]['email_id'] = []
                ...
                for face_box, face_embedding, face_pose in zip(face_boxes, face_embeddings, face_poses):
                    ...
                    if len(config.ROI) == 4 and is_centroid_in_roi(face_box, config.ROI):
                        ...
                        if len(face_box) > 0:
                            ...
                            name, employee_id, prediction, distance, email_id = self.match_embedding(face_embedding)
                            ...
                            self.current_trackers[lane_id][track_id]['face_email_id'].append(face_email)
                        ...
                        for track_id, preds in predictions.items():
                            if track_id >= 0:
                                if len(preds['face_boxes']) > 0 and len(preds['face_boxes'][-1]) == 4:
                                    ...
                                    user_mail_id = preds['face_email_id'][min_distance_index]
                                    employee_id = preds['face_ids'][min_distance_index]
                                    
                                    # if status == 2:
                                    #     text = f'{name} - Blacklisted'
                                    # else:    
                                    #     text = f'{name}'
                                    ...
                                    # draw_rectangle_with_text(img, (x1, y1), (x2, y2), color, text=name)
                                    ...
                                    if name == "Unknown":
                                        ...
                                        if name not in last_api_call_time or current_time - last_api_call_time[name]['time'] > 10:  # 10 seconds for demonstration
                                            ...
                                            image_path = f'{config.DEVICE}_{datetime.datetime.now().timestamp()}_{track_id}.jpg'
                                            cv2.imwrite(f'data/img/{image_path}', img)
                                            ...
                                            payload = {"email":user_mail_id, 'location_id': location_id}

                                            # Send the POST request with the payload and image separately
                                            response = requests.post(url, data=payload, files={'image': open(f'data/img/{image_path}', 'rb')})
                                            response_data = json.loads(response.text)
                                            print(response_data)
                                            try:
                                                # Update the last call time and state
                                                last_api_call_time[name] = {
                                                    'time': current_time,
                                            ...
                                            # # Check the last known state if within the 5 minutes window
                                            # if state_key in last_api_call_time:
                                            #     text = name + " -> " + last_api_call_time[state_key]
                                            # else:
                                            #     text = name + " -> Processing"
                                        
                                             
                                    draw_rectangle_with_text(img, (x1, y1), (x2, y2), color, text=text)
                                    # print(last_api_call_time)
